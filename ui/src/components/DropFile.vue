<template>
    ...
    <div class="dropArea" @drop.prevent @drop="handleDrop($event)" aria-live="polite" v-on:click.stop="checkFile()">  
        ...
        <div v-if="uploadedFiles.length > 0" class="cta">
            ...
            <button class="btn btn-lg btn-primary btn-pill btn-cta"  v-on:click="treestamp()" v-html="this.lb_12 + ' tree'"></button>
            <button class="btn btn-lg btn-primary btn-pill btn-cta"  v-on:click="singlestamp()" v-html="this.lb_12 + ' single'"></button>
            <!--<button class="btn btn-lg btn-primary btn-pill btn-cta"  v-on:click="batchstamp()" v-html="this.lb_12 + ' batch'"></button>-->
        </div>
...
export default {
    ...
    data: function() {
        return {
            loading: false,   
            limit: 100, // CAMBIAR ESTO SI SE PUEDE MAS DE 10 ARCHIVOS         
            uploadedFiles: [],
            verifyCounter: 0,
            allHashes: [],
            dragActive: false,
    ...
    methods: {
        ...
        treestamp() {
            ...
            let stampUrl = `${self.apiurl}/treestamp`
            self.loading = true
            axios.post(stampUrl, {
        ...
        singlestamp() {
            ...
            let stampUrl = `${self.apiurl}/singlestamp`
            self.loading = true
            axios.post(stampUrl, {
                hashes: self.allHashes
            }).then(function(response)
        ...
        uploadFiles: function(f) {            
            ...
            for (var i = 0; i < f[0].length; i++) {
                loadFile(f[0][i]);
                //console.log(f[0][i])
                self.formData.append('files[' + i + ']', f[0][i]);
        ...
        batchstamp(){
            ...
            let stampUrl = `${self.apiurl}/batchstamp`
            self.loading = true
            console.log(self.formData)
            axios.post(stampUrl, 
                self.formData,
